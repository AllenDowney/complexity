...
\begin{figure}
\centerline{\includegraphics[width=3.0in,height=1.5in]{figs/rule-50-10.pdf}}
\caption{Caption.}
\end{figure}
...
\begin{figure}
\centerline{\includegraphics[height=1.5in]{figs/array.pdf}}
\caption{Caption.}
\end{figure}
...
\begin{figure}
\centerline{\includegraphics[width=3.0in,height=1.5in]{figs/rule-30-100.pdf}}
\caption{Caption.}
\end{figure}
...
\begin{figure}
\centerline{\includegraphics[width=3.0in,height=1.5in]{figs/rule-110-100.pdf}}
\caption{Caption.}
\end{figure}
...
\begin{figure}
\centerline{\includegraphics[height=1.5in]{figs/fractal-254-4.pdf}}
\caption{Caption.}
\end{figure}
...
\begin{figure}
\centerline{\includegraphics[height=1.5in]{figs/fractal-254-8.pdf}}
\caption{Caption.}
\end{figure}
...
Worlds, so each ant has the attributes \texttt{x} and \texttt{y} to
...
%\begin{figure}
%\centerline{\includegraphics[width=3.0in]{figs/DirectedGraphExample.png}}
%\caption{Caption.}
%\end{figure}
...
When you
started your hypothetical research on \emph{topica obscura},
...
\section{Is Wikipedia scale-free?}

\begin{figure}
\centerline{\includegraphics[width=3.0in]{figs/BAResults.png}}
\caption{The graphs from Barabasi and Albert's 1999 paper show the
  scale-free structure of a few undirected graphs.}
\label{fig.BAGraph}
\end{figure}

In Barabasi and Albert's paper about scale-free
networks\footnote{Barabasi, Albert-Laszlo and Albert, Reka, Emergence
  of Scaling in Random Networks; Science, Vol. 286, 15
  October 1999.}, they analyze the underlying undirected graph of
citations in scientific articles.

They find that these graphs are scale-free; that is, the distribution
of node degree is long-tailed (see Section~\ref{}).  Figure~\ref{fig.BAGraph}
shows their results.  Each figure shows
 $k$ is the average connectivity, or degree, of the vertices in the graph, and $P(k)$ is the proportion of vertices in the graph that have this degree. Plotted on a log-log scale, scale-free networks tend to show long-tailed distributions. This is because a few vertices have the highest degree in the graph.

\begin{figure}
\centerline{\includegraphics[width=3.0in]{figs/WikipediaPowerLawGraph.png}}
\caption{The distribution function of connectivities for various large subgraphs of Wikipedia. (A) Index of India-related articles with $N = 5,342$ vertices and average connectivity $ k = 53.56$.  (B) Index of Mathematics articles starting with C, with $N = 3813$ and average connectivity $k = 4.95$. (C) Index of Mathematics articles starting with S, with $N = 3950$ and average connectivity $k = 4.85$. }
\label{fig.PunchlineGraph}
\end{figure}

To determine the structure of the subgraphs of Wikipedia, we looked at
the largest graphs and tried to recreate the results from Barabasi and
Albert's 1999 paper (seen in Figure \ref{fig.BAGraph}).

Comparing Figure \ref{fig.BAGraph} to Figure \ref{fig.PunchlineGraph},
we see that the general structure of both is similar. This suggests
that the subgraphs we examined are characteristically scale-free.  In
addition, it means that Wikipedia graphs are well-modeled by the
Barabasi-Albert small world graph model.

But what does this really mean? The Barabasi-Albert model of
small-world graphs is especially good at modeling preferential
attachment, the phenomenon where a few of the vertices have the
highest degree. The fact that both undirected graphs and the directed
graphs can have a scale-free distribution of degrees means that
preferential attachment is independent of the directedness of the
graph.


\begin{figure}
\centerline{\includegraphics[width=3.0in]{figs/AllThreeScaleFree.png}}
...
This independence is further demonstrated by Figure
\ref{fig.AllThree}, which shows the degree distribution of the in,
out, and total degrees of the ``Index of India-related articles''
subgraph. All three distributions are scale-free, which means that few
vertices have a high in-degree and few vertices have a high
out-degree. This corresponds with our findings that preferential
attachment is independent of the directedness of a graph.

\section{Conclusion}

Since Wikipedia subgraphs display a tendency towards preferential
attachment, we propose another explanation for why you keep landing on
the same articles. Perhaps there is not a knot in the articles you are
reading, and in fact, that set of articles is very well connected to
the rest of the graph. Since subgraphs of Wikipedia are small-world, a
small set of vertices will have the highest degrees in the graph. That
means that if you take a random walk through the graph, the odds
you'll encounter these vertices are very high.

Barabasi and Albert's example of undirected graph of citations in
science articles indicates that, even if you are traversing an
undirected graph, you may encounter the same vertices over and over,
without there being a knot. This small set of articles may be cited
the most, so in your research for a certain topic, you're very likely
to see these articles multiple times.

What does this mean for your research project? Whether you're browsing
Wikipedia or JSTOR, you'll end up seeing the same highly-referenced
nodes multiple times. In many ways, this can be frustrating when
looking for a specific or isolated article, but it is a result of the
small world nature of the graphs that represent the information you're
seeking.



\chapter{The Volunteer's Dilemma and the Norms Game}
...
\begin{quote}
You are a prairie dog assigned to guard duty with several other prairie dogs from your town.  When you see a predator coming, you have two choices: sound the alarm or remain silent.  If you sound the alarm, you help ensure the safety of the other prairie dogs, but you also encourage the predator to come after you.  If you remain silent and someone else sounds the alarm, your safety is better ensured.  If all the guards remain silent, the predator will come into the town and eat some of the unsuspecting prairie dogs within, possibly including you and the other guards.  What should you do when you see a predator?
\end{quote}

This scenario is an example of the Volunteer's Dilemma, a game similar to the Prisoner's Dilemma discussed in Section 9.5.  In the Prisoner's Dilemma each player has two options---cooperate and defect.  In the Volunteer's Dilemma each player also has two options---volunteer (sound the alarm in our prairie dog example) or ignore (remain silent).  If both players ignore, both pay a high cost.  However, if one player volunteers then the other player is better off ignoring.

In the Prisoner's Dilemma, both players, Alice and Bob, are better off if they both cooperate; however, since neither knows what the other will do, each independently comes to the conclusion that he or she should defect.  In the Volunteer's Dilemma, it is not immediately clear what situation is best for both players.  Looking at it from Bob's point of view: if Alice volunteers, Bob is better off ignoring; if Alice ignores, Bob is better off volunteering.  This does not provide a clear best strategy for Bob.  Alice reaches symmetric conclusions from her point of view: if Bob volunteers, she is better off ignoring; and if Bob ignores, she is better off volunteering.  This means we have two equivalent stable scenarios---Bob volunteers and Alice ignores, or Bob ignores and Alice volunteers.

In the prairie dog town, there are more than two guards.  As in the scenario with just Alice and Bob, only one player needs to volunteer to benefit the entire town.  We therefore have as many stable scenarios as we have guards---in each scenario, one guard volunteers and all others ignore.  If one guard is always going to volunteer, though, then there is little point in having multiple guards.  To make the situation more fair, the guards can distribute the burden of volunteering among themselves, collaborating based on a \textbf{mixed strategy}.  In a mixed strategy, a player chooses each option randomly.  Since we only have two options (ignore and volunteer), the player ignores with probability $\gamma$ and volunteers with probability $1 - \gamma$.  The optimal value for $\gamma$ is that where each player volunteers as little as necessary to produce the common good.

Marco Archetti investigates this optimal value of $\gamma$ in his paper ``The Volunteer's Dilemma and the Optimal Size of a Social Group.'' Our analysis in this section replicates Archetti's analysis. When each player volunteers with the optimal probability, the expected payoff of volunteering is the same as the expected payoff of ignoring.  Were the payoff of volunteering higher than the payoff of ignoring, the player would volunteer more often and ignore less; the opposite is true were the payoff of ignoring higher than the payoff of volunteering.  The expected payoff of volunteering or ignoring is the product of the likelihood the player will choose that option and the average benefit to the player of choosing that option, taking into account the other players' probable actions.  The optimal probability therefore depends on the costs and benefits of each option.  In fact, the optimal $\gamma$ for two individuals, $\gamma_2$, is related to the cost of volunteering, $c$, and the damage of nobody volunteering, $a$, such that $\gamma_2 = c / a$.  In the prairie dog example, where the damage of nobody volunteering is high, this means that the optimal $\gamma$ is small, though larger than 0.

If you transfer more prairie dogs to guard duty, there are more players to share the cost of volunteering, so intuitively the probability of each player ignoring should increase.  This certainly benefits each player---if the optimal probability of ignoring increases but the expected costs of volunteering and ignoring each stay the same, then each player has to pay the cost of volunteering less often and is better off.  Indeed, Archetti showed that $\gamma_N$ is equal to $\gamma_2^{1/(N-1)}$. The probability of everyone except you \emph{not} volunteering is $\gamma_N^{N-1} = \gamma_2$, since we multiply the probabilities of each individual not volunteering.

If everyone volunteers at the optimal probability, the probability of everyone except you ignoring stays consistent as the number of other players changes.  The probability that you ignore \emph{increases} as the number of other players increases, since $\gamma_N$ increases with $N$.  This means that, overall, as the size of the group increases, the probability that nobody volunteers increases.
...
The students at a small college sign an Honor Code upon entering freshman year. When a student breaks the Honor Code, those who witness the infraction face a version of the volunteer's dilemma. As a witness, you have two options: report the offender to be punished or remain silent. If you report the person, you help ensure that the Honor Code is followed and preserve the college's culture of honesty and integrity, but you also incur stress and are now required to follow through with the hassle. If you remain silent and someone else reports the person, you benefit without the stress and hassle. However, if everyone remains silent, the Honor Code is threatened because others may become, after seeing someone break it without getting punished, more likely to break the Honor Code.

Download and run \texttt{thinkcomplex.com/volunteersDilemma.py}, which contains a basic implementation of the Volunteer's Dilemma. The code plots the likelihood that nobody will volunteer with given values of $c$ and $a$ across a range of values for $N$.  Edit this code to investigate the Honor Code volunteer's dilemma.  How does the probability of nobody volunteering change as you modify the cost of volunteering, damage of nobody volunteering, and size of the population?
\end{exercise}

\section{Norms Game}

\begin{quote}
You are a high-school student about to take a very hard chemistry test.  You are tempted to write extra notes on your hand with answers to questions you suspect will be asked.  If you cheat, you benefit from a higher score, but other students suffer by your wrecking the curve.  Any student who sees you cheating has the choice to report you to the teacher, which disqualifies your curve-wrecking score but involves the embarrassing process of calling you out.  If you get away with cheating, others are more likely to cheat in the future.  How will your choice whether or not to cheat affect the future behavior of other students?
\end{quote}

This scenario is an example of the norms game.  Norms are standard behaviors practiced by a group.  When we model norms as a game, we discover the fittest behavior for the society---individuals whose behavior benefits them the most are likely to spread their behavior to others, so the society will tend towards this behavior over time.  This is a very different type of game from the Volunteer's Dilemma and the Prisoner's Dilemma, which are used to model an individual's optimal behavior in a given single scenario.  The norms game instead models a population's characteristic behavior over a period of time.

To bring in the temporal component of the norms game, our model is a genetic algorithm, based off of that described by Robert Axelrod in ``Promoting Norms: An Evolutionary Approach to Norms.''  Genetic algorithms model the process of natural evolution.  A population is initialized, usually randomly, by creating a group of individuals and assigning each individual random characteristics.  In our genetic algorithm, these characteristics are \emph{boldness}, the likelihood that the individual will cheat, and \emph{vengefulness}, the likelihood that the individual will report another individual he observes cheating.  Each individual in the genetic algorithm has a fitness level, which describes how well that individual is doing relative to others in the game.  This encompasses the effects of the \emph{reward} of cheating, the \emph{damage} from having another student cheat, the \emph{cost} of reporting a cheater, and the \emph{punishment} from being reported.  When an individual cheats, his fitness increases by \emph{reward} points, but the fitness of others decreases by \emph{damage} points.  Each individual who reports a cheater loses \emph{cost} fitness points and causes the cheater to lose \emph{punishment} fitness points.

At the end of each generation, individuals with the highest fitness levels have the most children in the next generation, whereas individuals with the lowest fitness levels have the fewest children.  The properties of each individual in the new generation are then mutated with a small probability to maintain variability in the population.

The norms game is a compound game consisting of two subgames, which we call ``defect-or-not'' and ``punish-or-not.''  In the first, each player decides whether or not to defect (or cheat), with probability equal to the \emph{boldness} value.  In the second, each individual decides whether or not to punish each cheater he sees cheating, with probability equal to the \emph{vengefulness} value.  These subgames are played several times within a generation so that each individual has several opportunities to defect and choose whether or not to punish defectors. This pseudocode describes our algorithm for the norms game:
...
Each player in our simulation of the norms game is represented by a \texttt{Player} object.  A \texttt{Player} has three attributes, \texttt{boldness}, \texttt{vengefulness}, and \texttt{fitness}.  When repopulating, each individual with \texttt{fitness} greater than one standard deviation above the mean has two children; each individual with \texttt{fitness} less than one standard deviation below the mean has zero children; others have one child.

Figure \ref{fig.normsRep} shows the result of many runs of the norms game after 300 generations.  There are two clear clusters in the data---one with low average \texttt{boldness} and one with high average \texttt{boldness}.  The first of these is societally preferable, as the majority of the population will not cheat, whereas in the second cluster the majority of the population will cheat most of the time.  When the average \texttt{boldness} is low ($\leq 0.2$), the average \texttt{vengefulness} is spread relatively uniformly between 0 and 0.8, which suggests that when the population is not bold, the level of vengefulness does not matter very much.  This is logical because if few to no people defect then opportunities to punish are rare and therefore the vengefulness value does not particularly influence an individual's fitness.  When the average \texttt{boldness} is high ($\geq 0.8$), the average \texttt{vengefulness} is consistently low.  The existence of this cluster is less intuitive, because the mean population fitness is highest if nobody defects. However, just as in the Prisoner's Dilemma, players are not concerned with the greater good or the mean population fitness---defecting benefits oneself and harms everyone else, which is a further advantage to the player. How then can we explain the low level of vengefulness?

\begin{figure}
  \begin{center}
    \includegraphics[width=0.75\textwidth]{figs/replicateNorms_stableBV_03.png}
  \end{center}
  \caption{Plot of stable states of the norms game after 300 generations.  Values used: reward = 3, damage = -1, cost = -2, punishment = -9, repetitions = 4.}\label{fig.normsRep}
\end{figure}

In the punish-or-not subgame, we can analyze whether or not each individual should punish a cheater.  Recall that we are considering relative fitness, so each individual wants to do well as compared to others in the population.  Each time a student punishes the cheater, the cheater receives an additional punishment $P$ and the punisher pays a cost $C$.  The mean fitness therefore decreases by $(P + C) / N$ each time another student punishes the cheater, which gives a relative benefit to every student who did not punish.  If you punish the cheater, the mean fitness decreases again by $(P + C) / N$, but you pay a cost of $C$.  Punishing is only beneficial to you if the cost you pay is less than the mean decrease in fitness; otherwise, you are losing relative to the other players.  Therefore, you should only punish if $(P + C) / N > C$.

This is only true for small values of $N$.  If every player only punishes when it would benefit him or herself, the player will only punish if the population size is very small.  The value of $N$ we used in the simulation was sufficiently large to encourage players not to punish, so we saw the cluster with high boldness and low vengefulness.  Like the results of the Prisoner's Dilemma and the Volunteer's Dilemma, this is disheartening because all players would be better off in overall fitness if none of them defected, yet the tendency is to encourage defection by not punishing.

\begin{exercise}

The implementation of the punish-or-not subgame described above is slightly different from the chemistry class cheating example, which has an implicit Volunteer's Dilemma.  In that example, the cheater received punishment $P$ once if at least one individual called him out but did not receive extra punishment if multiple individuals reported him.  Repeat the analysis of the punish-or-not subgame for the chemistry class example and determine in which situation you should punish fellow cheaters if punishment follows the Volunteer's Dilemma.
\end{exercise}
...
\begin{figure}
  ...
  \end{center}
  \caption{Contour plot of proportion of good outcomes by varying cost and punishment.}\label{fig.varyCP}
\end{figure}

It can be understood that a good outcome to a population is obtained when the proportion of players with low boldness is high and the proportion of players with high boldness is low. For the following analysis, we define a good outcome as the situation where, after 300 generations, the mean boldness is 0.4 or less. Figure \ref{fig.varyCP} shows how the proportion of good outcomes changes with varying cost and punishment. When we increase the cost of enforcing the norm, players who do not volunteer benefit even more than when the cost was lower from volunteerism of other players. This puts the volunteers at a much greater disadvantage compared to the non-volunteers, and, over time, more of the non-volunteers are reproduced than the volunteers---which means that the defectors are even less likely to be punished and that there would be an advantage to being bold. The population would then stabilize with a higher mean boldness, and an outcome that is less favorable. When we increase the punishment levied on cheaters, the players with high boldness---who would be the most likely to be punished---are at a greater disadvantage when punished. Over time, the population stabilizes with a lower mean boldness.

It's interesting to note that, if we begin at where cost is $-2$ and punishment is between $-7$ and $-8$, we obtain good outcomes with a probability $0.20$. To increase that probability to $0.80$, we can either lower the cost to $-1$ or increase the punishment to almost $-11$. Which option is better depends on the relative difficulty of implementing these two potential improvements.

\begin{exercise}

Analyze the defect-or-not subgame---under what circumstances should an individual defect?  Then download \texttt{thinkcomplex.com/normsGame.py} and determine what values of reward and damage, with our original values of cost and punishment, give a 50\% chance that the equilibrium will be with low boldness?  What about 70\% and 90\%?
\end{exercise}

\section{Conclusion}

We have now explored two new types of games which give us insight into different realms of human behavior.  The Volunteer's Dilemma, like the Prisoner's Dilemma, helps us understand the optimal choice for an individual in a given situation.  The norms game helps us understand how the behavior of a group of individuals will change over a period of time to favor optimal behaviors.  Note that in neither of these situations is the calculated optimal behavior necessarily the behavior which actually benefits all parties the most but rather represents a mathematical analysis of immediate benefit to each individual.

By putting simple games together we can get complex interactions which can help us study interesting properties of a population.  These games become more difficult to study through hand calculations, but by simulating the game circumstances we can determine how different sets of situations lead to unexpected behaviors.  For more information on game theory and different types of games, consult \url{http://en.wikipedia.org/wiki/Game_theory}.


\chapter{Disease transmission in social networks}

{\em Margaret-Ann Seger, Boris Taratutin, Eun Abe Kim}
...
\section{Disease Spread Models}
In \textit{The impacts of network topology on disease spread}, Mark Shirley describes a particular implementation of a disease spread model in an arbitrary network\footnote{To read more about Shirley's model, go to http://www.sciencedirect.com/science/article/pii/S1476945X05000280}. In the real world, when two people come into contact with each other, there is the potential of infecting one another. To model this, Shirley represents individuals  as nodes,  and represents the links between them as potential points for disease transmission. In Shirley's model, nodes can be in one of four states:  `healthy', `infected', `infectious', or `immune'. For the purposes of the model,  `immune' includes both deceased individuals and individuals who have been infected but recovered from the disease and cannot contract it again.

These states, coupled with the following state transitions, are the basis for our implementation of Shirley's model:
\begin{itemize}
    \item `Healthy' nodes adjacent to an infected node have a chance of becoming infected by infectious nodes (with probability $p_infection$)
    \item Infected nodes will eventually become infectious (after $latent_period$)
    \item Infectious nodes will eventually become immune (after $infectious_period$)
    \item Immune nodes can no longer infect other nodes or be infected themselves
\end{itemize}
...
In his 1999 paper, \textit{Emergence of Scaling in Random Networks}, Albert Barabasi introduces a method of preferential attachment for creating scale-free graphs and describes it as a network that is grown, where new nodes prefer to attach to nodes that already have a large number of connections. Barabasi argues that scale-free network topologies are useful for representing abstract systems like the World Wide Web and human social interactions, hence making scale-free graphs the logical choice as a base for our disease-spread model.



\section{Simulation}
Figure ~\ref{fig.disease_progression} shows Shirley's disease-transmission model on Barabasi's scale-free network.

\begin{figure}
\centerline{\includegraphics[width=0.8\textwidth]{figs/Disease_Progression.png}}
\caption{A plot of the number of people in each state of Shirley's disease spread model ('normal', 'infected', 'infectious', and 'immune') over the course of an epidemic. The simulation parameters were as follows: $n=500$, $p_{infection}=0.5$, $latent_period=3$, $infectious_period=6$). Each line represents the number of people in that given state at any point in time.}
...
\textbf{Exercise 1:} \textit{Try using the Python file DiseaseSpreadModel.py to run the simulation with different parameters. How do different parameters affect the spread of disease? Note that each time the simulation is run, the generated graph has different shape and form from the one before. How does this randomness in the simulation affect the results? Run the simulation an appropriate number of times to see some convergence in results: how many times do you have to run it?}



\section{Identifying the `key' Nodes in an Epidemic}
Alex Ng and Janet Efstathiou, in \textit{The Structural Robustness of Complex Networks}, describe a technique whereby each node in a network is systematically removed, the disease-spread simulation run, and the final size of the epidemic calculated. We incorporated the same technique in our simulation to measure the effect of quarantining any given node on the final epidemic size.

We ran the simulation 500 times to account for variability in the network topology due to randomness in scale-free network `growth' and in the disease spread model. We averaged these results to calculate an epidemic size for each removed node, and plotted the results in Figure ~\ref{fig.node_removal_simulation}.

\begin{figure}
\centerline{\includegraphics[width=0.9\textwidth]{figs/Node_Removal_Simulation.png}}
\caption{A plot of change in epidemic size for each node removed, for a randomly-generated scale-free graph topology. In this scenario, there are a significant number of nodes that could drastically reduce the size of the epidemic.}
\label{fig.node_removal_simulation}
\end{figure}

In Figure ~\ref{fig.node_removal_simulation}, the nodes highlighted in orange are the ones that, if removed, produce the largest reduction in epidemic size. We call these `key' nodes.




\section{Finding a More General Node-Removal Method}
...
In Figure ~\ref{fig.node_removal_comparison}, we ran a comparison that, one at a time, removed nodes from the network, using each of these three methods for choosing which nodes to remove, calculated the resulting epidemic size, and plotted the results.

\begin{figure}
\centerline{\includegraphics[width=1.0\textwidth]{figs/Node_Removal_Comparison.png}}
\caption{A plot of epidemic size as a function of how many nodes were removed. Three different techniques were used for selecting which order to remove nodes---random (worst-case), exhaustive search (best-case) and based on degree.}
\label{fig.node_removal_comparison}
\end{figure}

Figure ~\ref{fig.node_removal_comparison} shows that selecting nodes to vaccinate based on their degree is just as good as selecting them by exhaustive search, and far better than a random selection method. Furthermore, figure ~\ref{fig.node_removal_comparison} shows that, with relatively few highly-connected nodes removed, the epidemic size drops dramatically. The implications of this are significant: in a college of 2,000 students, if an epidemic broke out, you could save 1000 students by vaccinating the top 100 most connected students.



\section{Tying Back to the Real World}
...
Though campus social media gurus may have a thousand or more digital `friends' and `followers', they would not be the key students to vaccinate in case of an epidemic. Rather, those students who are in close physical proximity to large numbers of people on a day-to-day basis should be targeted.
...
\textbf{Exercise 2:} \textit{Read about the idea of bridges and see if you can find a data set that could help you identify what some real-world bridges might be. Some possible people you might think of are flight attendants or store clerks, who come into physical contact with a large number of people  that may not normally interact with each other. Notice that you never talk about a specific individual as a bridge but rather a group of people, whether it be defined by their profession or the place they spend time in, etc.}

\textbf{Exercise 3:} \textit{When we made the decision to pursue node degree as a generalizable strategy for identifying key nodes, there were many other characteristics we could have explored. These include:}

\begin{enumerate}
    \item \textbf{Node connectivity}: Node connectivity is defined to be, ``The minimum number of nodes which need to be removed to disconnect the remaining nodes from each other.'' (Wikipedia).
    \item \textbf{Network diameter}: A network's diameter is the maximum of all of the shortest paths between any two nodes in the graph.
    \item \textbf{Average shortest path length in network}: The average shortest path length is the average of all of the shortest path lengths between any two nodes in the network.
    \item \textbf{Network clustering coefficient}: A network's clustering coefficient is, ``a measure of degree to which nodes in a graph tend to cluster together.'' (Wikipedia).
    \item \textbf{Link Load}: Link load is, ``The amount of traffic carried by a link, or by the most heavily loaded link in the system'' (http://parallel.ru/info/reference/hpccgloss.html)
\end{enumerate}
...
Barabasi, Albert. \textit{Emergence of Scaling in Random Networks}. 1999. \\
Downey, Allen. \textit{Complexity and Computation}. 2011\\
Galvani and Medlock \textit{Optimizing Influenza Vaccine Distribution}. 2009. \\
Ng and Efstathiou. \textit{The Structural Robustness of Complex Networks}. 2006. \\
Shirley, Mark. \textit{The impacts of network topology on disease spread}. 2005.
