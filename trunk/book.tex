...
When new students arrive on campus each fall, many colleges experience
a so-called ``Freshman Plague,'' a flu-like disease that sweeps
through the school.  Occasionally, for illnesses such as norovirus or
the H1N1 outbreak of 2009, students have been quarantined or entire
campuses shut down.

Of course, quarantines and school closures are disruptive and
expensive.  But what if the university could predict how the disease
would spread and take preventative actions with those individuals most
likely to perpetuate the spread of the disease? 

In this case study, we use the structure of social networks to
evaluate the effect of preemptive strategies, including vaccination
and quarantine, on the spread of disease.


\section{A disease model}

In ``The impacts of network topology on disease spread,'' Mark
Shirley and Steve Rushton describe a model of disease spread
in an arbitrary network.  You can download the paper from
\url{http://www.sciencedirect.com/science/article/pii/S1476945X05000280}.

In the real world, when two people come into contact with each other,
there is the potential of infecting one another. To model this,
Shirley and Rushton represents individuals as nodes in a graph, and
opportunities for infection as edges or ``links''.

In their model, nodes can be in one of four states:
healthy, infected, infectious, or immune. 

\begin{itemize}

    \item A healthy node adjacent to an infected node 
      becomes infected with probability
      \verb"p_infection".

    \item An infected node becomes infectious after
      \verb"latent_period".

    \item An infectious node becomes immune after
      \verb"infectious_period".

    \item Immune nodes can not infect other nodes or be infected.
\end{itemize}

An immune node might represent someone who has recovered and
cannot be infected again, or someone who died.  In the model there
is no difference between these outcomes; of course, in the real world
there is.

Shirley and Rushton test their model on \Erdos-\Renyi~random graphs,
Watts-Strogatz small-world graphs, and \Barabasi-Albert scale-free
networks.  We focus on scale-free networks because, of these
alternatives, they are the best model of the topology of social
networks (see Section~\ref{scale.free}).
...
\caption{The number of people in each state (healthy, infected, infectious, and
  immune) over the course of an epidemic. }
\label{fig.disease_progression}
\end{figure}

We implemented Shirley and Rushton's model and ran it on a
scale-free network with 500 nodes, with parameters
\verb"p_infection" $=0.5$,
  \verb"latent_period" $=3$, \verb"infectious_period" $=6$.

Figure ~\ref{fig.disease_progression} shows the results.
The lines represent the number of healthy, infected, infectious, and
immune people over time. 

In this example, the result is a complete pandemic---everyone gets infected.
The shape of the immune curve is a sigmoid: growth is approximately
exponential at first, but slows as the number of immune people increases.



\begin{exercise}

Download our code from \url{thinkcomplex.com/DiseaseSpreadModel.py}
and run the simulation
with different parameters. How does each parameters affect the
spread of disease?
\end{exercise}
