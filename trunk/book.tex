...
\item In a system, S, we see a behavior, B, that warrants explanation.
...
\item By simulation or mathematical derivation, we show that the model
  exhibits a behavior, B', that is analogous to B.

\item We conclude that S exhibits B {\em because} S is similar to M, M
  exhibits B', and B' is similar to B.
...
B', B'' and B''', that are similar to B in different ways.

In that case, which model explains B?
