...
fascination with complexity systems.  The problem with data structures
...
read about topics in complexity, implement experiments in
...
\item[Further reading] Throughout the book, I include pointers to
  ...
  Wikipedia\footnote{Some professors have an allergic reaction to
    Wikipedia, on the grounds that students rely too heavily on an
    ...
    are unrealiable, that that the readers won't come back!  (see
...
systems that contain many interacting components.  
...
soon, but if you are impatient, you can get a preview at
...
Both systems are complex, and both models are based on simplifications;
...
including prediction, explanation, and design.  So one of the goals of
this book is to explain how.


\section{Paradigm shift?}

When I describe this book to people, I am often asked if the new kind
...
particular that all fireflies can see all other fireflies.
...
can explore (in ways that would be difficult or impossible with
mathematical analysis) the conditions that allow or prevent
synchronization.

I am a computer scientist, so my enthusiasm for computational
...
analysis; conversely, models of complexity systems are often
...
too seriously.  By describing these trends, I don't mean to deprecate
...
cheek, {\em Homo economicus}.  Research based on this model was
highly-regarded for several decades, especially if it involved
mathematical virtuosity.  More recently, this model is regarded
with more skepticism, and models that include imperfect information and
bounded rationality are hot topics.


\section{A new kind of model}

Complex models are often appropriate
for different purposes and interpretations:
...
would accept that electrons are real things that exist.  Instrumentalism
is the view that models can be useful even if the entities they
postulate don't exist.  
...
the electrons in an atom.  Holism is the view that
some phenomena that appear at the system level do not exist at the
level of components, and cannot be explained in component-level
terms.

\end{description}

I am worried about introducing some of these ideas now because it
is hard to present them briefly without oversimplifying.  We
get back to explanatory models in Chapter~\ref{scale-free},
instrumentalism in Chapter~\ref{life} and
holism in Chapter~\ref{soc}.


\section{A new kind of engineering}

I have been talking about complex systems in the context of science,
but complexity is also the cause, and effect, of gradual
...
\item[top-down $\rightarrow$ bottom-up] In social, political and
  ...
  are the canonical example of a hierarchical structure, are moving
  toward devolved command and control.

\item[analysis $\rightarrow$ computation] In classical engineering,
  the space of feasible designs was limited by our capability for
  ...
  can be imagined.  Frank Gehry's Guggenheim Museum Bilbao is a
  salient example.
...
\item[design $\rightarrow$ search] Engineering is sometimes described
  ...
  genetic algorithms can explore large design spaces and discover
  solutions human engineers would not have imagined.
...
\item[Aristotelian logic $\rightarrow$ many-valued logic] In
  ...
  Kosko discusses the emergence of these systems in {\em Fuzzy
    Thinking}.
...
\item[objective $\rightarrow$ subjective] The Enlightenment, and
  ...
  is, truths that are independent of the people that who them.  20th
  ...
  even ``hard sciences'' and mathematics.
...
These trends are neither universal nor complete, but the center of
...
For example, the use of highly abstracted models is more acceptable
now because of the diminished expectation that 
there should be a ``Theory of Everything.''  Conversely,
developments in complex systems challenge determinism and the
related concept of physical law. 

This chapter is intended to provide an overview of the themes
coming up in the book.  Of course, not all of it will make sense before
you see the examples in the rest of the book.  When you get to
the end of the book, you might find it helpful to come back and
read this chapter again.

If you already read the book and this is your second iteration, you
should stop reading and get to work on your case study (see
Appendix~\ref{submissions}).
...
\begin{figure}
\centerline{\includegraphics[width=5.5in]{figs/model.pdf}}
...
\begin{figure}
\centerline{\includegraphics[width=5.5in]{figs/model2.pdf}}
...
computational shortcut, but there is a qualitative difference.
...
beyond which we can't compute predictions at all.
...
The status of emergent properties is under contention, so critical
inquiry is appropriate. When we see an apparently emergent property,
...
we have seen in this book, and the principle of computational
...
These examples suggest a new approach to several old and challenging
...
determined.  I won't try to present the many arguments about free will
systematically, but I will mention two:
...
The complex systems we have seen in this book suggest the alternative
...
forward, a person has free will even though neurons don't.
...
What is the {\em system reply} to the Chinese Room argument?
How does what you have learned about complexity science influence
your reaction to the system response?

\end{exercise}

You have reached the end of the book.  Congratulations!  When you read
Chapter~\ref{overview}, some of the topics might not have made sense.
You might find it helpful to read that chapter again now.  Then get to
work on your case study (see Appendix~\ref{submissions}).
