...
You are a prairie dog assigned to guard duty with several other
...
also encourage the predator to come after you.  If you remain silent
and someone else sounds the alarm, your safety is better ensured.  If
all the guards remain silent, the predator will come into the town and
eat some of the unsuspecting prairie dogs within, possibly including
you and the other guards.  What should you do when you see a predator?
\end{quote}

This scenario is an example of the Volunteer's Dilemma, a game similar
to the Prisoner's Dilemma discussed in Section 9.5.  In the Prisoner's
Dilemma each player has two options---cooperate and defect.  In the
Volunteer's Dilemma each player also has two options---volunteer
(sound the alarm in our prairie dog example) or ignore (remain
silent).  If both players ignore, both pay a high cost.  However, if
one player volunteers then the other player is better off ignoring.

In the Prisoner's Dilemma, both players, Alice and Bob, are better off
...
should defect.  In the Volunteer's Dilemma, it is not immediately
clear what situation is best for both players.  Looking at it from
Bob's point of view: if Alice volunteers, Bob is better off ignoring;
if Alice ignores, Bob is better off volunteering.  This does not
provide a clear best strategy for Bob.  Alice reaches symmetric
conclusions from her point of view: if Bob volunteers, she is better
off ignoring; and if Bob ignores, she is better off volunteering.
This means we have two equivalent stable scenarios---Bob volunteers
...
to benefit the entire town.  We therefore have as many stable
scenarios as we have guards---in each scenario, one guard volunteers
and all others ignore.  If one guard is always going to volunteer,
though, then there is little point in having multiple guards.  To make
the situation more fair, the guards can distribute the burden of
volunteering among themselves, collaborating based on a \textbf{mixed
  strategy}.  In a mixed strategy, a player chooses each option
randomly.  Since we only have two options (ignore and volunteer), the
player ignores with probability $\gamma$ and volunteers with
probability $1 - \gamma$.  The optimal value for $\gamma$ is that
where each player volunteers as little as necessary to produce the
common good.

Marco Archetti investigates this optimal value of $\gamma$ in his
paper ``The Volunteer's Dilemma and the Optimal Size of a Social
Group.'' Our analysis in this section replicates Archetti's
analysis. When each player volunteers with the optimal probability,
...
payoff of volunteering.  The expected payoff of volunteering or
ignoring is the product of the likelihood the player will choose that
option and the average benefit to the player of choosing that option,
taking into account the other players' probable actions.  The optimal
probability therefore depends on the costs and benefits of each
option.  In fact, the optimal $\gamma$ for two individuals,
$\gamma_2$, is related to the cost of volunteering, $c$, and the
damage of nobody volunteering, $a$, such that $\gamma_2 = c / a$.  In
the prairie dog example, where the damage of nobody volunteering is
high, this means that the optimal $\gamma$ is small, though larger
than 0.

If you transfer more prairie dogs to guard duty, there are more
players to share the cost of volunteering, so intuitively the
probability of each player ignoring should increase.  This certainly
benefits each player---if the optimal probability of ignoring
increases but the expected costs of volunteering and ignoring each
stay the same, then each player has to pay the cost of volunteering
less often and is better off.  Indeed, Archetti showed that $\gamma_N$
is equal to $\gamma_2^{1/(N-1)}$. The probability of everyone except
you \emph{not} volunteering is $\gamma_N^{N-1} = \gamma_2$, since we
multiply the probabilities of each individual not volunteering.

If everyone volunteers at the optimal probability, the probability of
everyone except you ignoring stays consistent as the number of other
players changes.  The probability that you ignore \emph{increases} as
the number of other players increases, since $\gamma_N$ increases with
$N$.  This means that, overall, as the size of the group increases,
...
The students at a small college sign an Honor Code upon entering
freshman year. When a student breaks the Honor Code, those who witness
the infraction face a version of the volunteer's dilemma. As a
witness, you have two options: report the offender to be punished or
remain silent. If you report the person, you help ensure that the
Honor Code is followed and preserve the college's culture of honesty
and integrity, but you also incur stress and are now required to
follow through with the hassle. If you remain silent and someone else
reports the person, you benefit without the stress and
hassle. However, if everyone remains silent, the Honor Code is
threatened because others may become, after seeing someone break it
without getting punished, more likely to break the Honor Code.

Download and run \texttt{thinkcomplex.com/volunteersDilemma.py}, which
...
volunteering, damage of nobody volunteering, and size of the
population?
\end{exercise}
...
You are a high-school student about to take a very hard chemistry
test.  You are tempted to write extra notes on your hand with answers
to questions you suspect will be asked.  If you cheat, you benefit
from a higher score, but other students suffer by your wrecking the
curve.  Any student who sees you cheating has the choice to report you
to the teacher, which disqualifies your curve-wrecking score but
involves the embarrassing process of calling you out.  If you get away
with cheating, others are more likely to cheat in the future.  How
will your choice whether or not to cheat affect the future behavior of
other students?
\end{quote}

This scenario is an example of the norms game.  Norms are standard
behaviors practiced by a group.  When we model norms as a game, we
discover the fittest behavior for the society---individuals whose
behavior benefits them the most are likely to spread their behavior to
others, so the society will tend towards this behavior over time.
This is a very different type of game from the Volunteer's Dilemma and
the Prisoner's Dilemma, which are used to model an individual's
optimal behavior in a given single scenario.  The norms game instead
models a population's characteristic behavior over a period of time.

To bring in the temporal component of the norms game, our model is a
genetic algorithm, based off of that described by Robert Axelrod in
``Promoting Norms: An Evolutionary Approach to Norms.''  Genetic
algorithms model the process of natural evolution.  A population is
initialized, usually randomly, by creating a group of individuals and
assigning each individual random characteristics.  In our genetic
algorithm, these characteristics are \emph{boldness}, the likelihood
that the individual will cheat, and \emph{vengefulness}, the
likelihood that the individual will report another individual he
observes cheating.  Each individual in the genetic algorithm has a
fitness level, which describes how well that individual is doing
relative to others in the game.  This encompasses the effects of the
\emph{reward} of cheating, the \emph{damage} from having another
student cheat, the \emph{cost} of reporting a cheater, and the
\emph{punishment} from being reported.  When an individual cheats, his
...
levels have the most children in the next generation, whereas
individuals with the lowest fitness levels have the fewest children.
The properties of each individual in the new generation are then
mutated with a small probability to maintain variability in the
population.

The norms game is a compound game consisting of two subgames, which we
call ``defect-or-not'' and ``punish-or-not.''  In the first, each
player decides whether or not to defect (or cheat), with probability
equal to the \emph{boldness} value.  In the second, each individual
decides whether or not to punish each cheater he sees cheating, with
probability equal to the \emph{vengefulness} value.  These subgames
are played several times within a generation so that each individual
has several opportunities to defect and choose whether or not to
punish defectors. This pseudocode describes our algorithm for the
norms game:
...
Figure~\ref{fig.normsRep} shows the result of many runs of the norms
game after 300 generations.  There are two clear clusters in the
data---one with low average \texttt{boldness} and one with high
average \texttt{boldness}.  The first of these is societally
preferable, as the majority of the population will not cheat, whereas
in the second cluster the majority of the population will cheat most
of the time.  When the average \texttt{boldness} is low ($\leq 0.2$),
the average \texttt{vengefulness} is spread relatively uniformly
between 0 and 0.8, which suggests that when the population is not
bold, the level of vengefulness does not matter very much.  This is
logical because if few to no people defect then opportunities to
punish are rare and therefore the vengefulness value does not
particularly influence an individual's fitness.  When the average
\texttt{boldness} is high ($\geq 0.8$), the average
\texttt{vengefulness} is consistently low.  The existence of this
cluster is less intuitive, because the mean population fitness is
highest if nobody defects. However, just as in the Prisoner's Dilemma,
players are not concerned with the greater good or the mean population
fitness---defecting benefits oneself and harms everyone else, which is
...
It can be understood that a good outcome to a population is obtained
when the proportion of players with low boldness is high and the
proportion of players with high boldness is low. For the following
analysis, we define a good outcome as the situation where, after 300
generations, the mean boldness is 0.4 or less. Figure~\ref{fig.varyCP}
shows how the proportion of good outcomes changes with varying cost
and punishment. When we increase the cost of enforcing the norm,
...
volunteers---which means that the defectors are even less likely to be
...
outcome that is less favorable. When we increase the punishment levied
...
It's interesting to note that, if we begin at where cost is $-2$ and
punishment is between $-7$ and $-8$, we obtain good outcomes with a
probability $0.20$. To increase that probability to $0.80$, we can
either lower the cost to $-1$ or increase the punishment to almost
$-11$. Which option is better depends on the relative difficulty of
implementing these two potential improvements.

\begin{exercise}

Analyze the defect-or-not subgame---under what circumstances should an
individual defect?  Then download
\texttt{thinkcomplex.com/normsGame.py} and determine what values of
reward and damage, with our original values of cost and punishment,
give a 50\% chance that the equilibrium will be with low boldness?
What about 70\% and 90\%?
\end{exercise}

\section{Conclusion}

We have now explored two new types of games which give us insight into
different realms of human behavior.  The Volunteer's Dilemma, like the
Prisoner's Dilemma, helps us understand the optimal choice for an
individual in a given situation.  The norms game helps us understand
how the behavior of a group of individuals will change over a period
of time to favor optimal behaviors.  Note that in neither of these
situations is the calculated optimal behavior necessarily the behavior
which actually benefits all parties the most but rather represents a
mathematical analysis of immediate benefit to each individual.

By putting simple games together we can get complex interactions which
can help us study interesting properties of a population.  These games
become more difficult to study through hand calculations, but by
simulating the game circumstances we can determine how different sets
of situations lead to unexpected behaviors.  For more information on
game theory and different types of games, consult
...
Each fall, millions of college kids go back to school. And each fall,
numerous colleges experience the fabled ``Freshman Plague'' where a
large percentage of the incoming students contract an unknown disease
(usually some variety of influenza) that sweeps through the class like
a plague.

University health offices often urge students to follow stringent
health and safety guidelines to fight the spread of
sickness. Occasionally, for illnesses such as norovirus or the H1N1
`Swine Flu' outbreak of 2009, students have been quarantined or entire
campuses shut down. Quarantines and school closures are traumatic and
disruptive, as well as expensive.

Imagine however, the university could predict how the disease would
spread and take preventative actions with those individuals most
likely to perpetuate the spread of the disease. These pre-emptive
actions might take the form of immunization, quarantine, or extra
health precautions.

In this chapter, we develop a way to pinpoint the most crucial
students to target for such preventative measures, whose removal or
vaccination minimize the spread of disease.
...
In \textit{The impacts of network topology on disease spread}, Mark
Shirley describes a particular implementation of a disease spread
model in an arbitrary network\footnote{To read more about Shirley's
  model, go to
  http://www.sciencedirect.com/science/article/pii/S1476945X05000280}. In
the real world, when two people come into contact with each other,
there is the potential of infecting one another. To model this,
Shirley represents individuals as nodes, and represents the links
between them as potential points for disease transmission. In
Shirley's model, nodes can be in one of four states: `healthy',
`infected', `infectious', or `immune'. For the purposes of the model,
`immune' includes both deceased individuals and individuals who have
been infected but recovered from the disease and cannot contract it
again.

These states, coupled with the following state transitions, are the
basis for our implementation of Shirley's model:
...
Shirley tested his model on random, small-world, and scale-free
networks.  We chose to use a scale-free network as this topology most
closely mimics the structure of a human network.

In his 1999 paper, \textit{Emergence of Scaling in Random Networks},
Barabasi and Albert introduce a method of preferential attachment for
creating scale-free graphs and describes it as a network that is
grown, where new nodes prefer to attach to nodes that already have a
large number of connections. Barabasi argues that scale-free network
topologies are useful for representing abstract systems like the World
Wide Web and human social interactions, hence making scale-free graphs
the logical choice as a base for our disease-spread model.



\section{Simulation}

Figure ~\ref{fig.disease_progression} shows Shirley's
disease-transmission model on a Barabasi-Albert scale-free network.
...
The lines represent the number of normal, infected, infectious, and
immune people at any given point in time. Looking at Figure 1, we see
that the epidemic seems to grow as a logistic function. This is
consistent with what happens in the real world where 'hubs'---very
well-connected nodes--- act to expedite the spread of disease.

\begin{exercise}
Try using the Python file DiseaseSpreadModel.py to run the simulation
with different parameters. How do different parameters affect the
spread of disease? Note that each time the simulation is run, the
generated graph has different shape and form from the one before. How
does this randomness in the simulation affect the results? Run the
simulation an appropriate number of times to see some convergence in
results: how many times do you have to run it?
\end{exercise}
...
Alex Ng and Janet Efstathiou, in \textit{The Structural Robustness of
  Complex Networks}, describe a technique whereby each node in a
network is systematically removed, the disease-spread simulation run,
and the final size of the epidemic calculated. We incorporated the
same technique in our simulation to measure the effect of quarantining
any given node on the final epidemic size.

We ran the simulation 500 times to account for variability in the
network topology due to randomness in scale-free network `growth' and
in the disease spread model. We averaged these results to calculate an
epidemic size for each removed node, and plotted the results in Figure
~\ref{fig.node_removal_simulation}.
...
In Figure ~\ref{fig.node_removal_simulation}, the nodes highlighted in
orange are the ones that, if removed, produce the largest reduction in
epidemic size. We call these `key' nodes.
...
Up to this point, all of the results we have attained have used
techniques that require perfect knowledge of the network. However,
having a strategy that depends on a highly specific network is not the
optimal solution for determining what will happen in the real
world---as it is subject to small variances in network topology or
simulation parameters, and runs the risk of being too much of an
abstraction from the real world.

In a real-world scenario like a virus outbreak on a college campus,
disease spreads through networks that are inherently complex and
cannot be represented by a simple graph topology---making it very
difficult, if not impossible, to discern individuals to vaccinate .

The natural extension then is to identify the general characteristics
of these `key' nodes, so that we can identify which nodes to
vaccinate, without needing perfect knowledge of the underlying network
structure. We concluded that a node's degree is a good and simple
measure of `importance'.

To evaluate the effectiveness of using node degree as a method for
determining key nodes, we ran a test that plots epidemic size as a
function of the number of nodes deleted from the network for three
different node-removal techniques: a `worst-case' random-selection
method, a `best case' exhaustive search method, and the node
degree-based selection method we wanted to test. We hypothesized that
degree-based selection would be more effective than random selection,
but less effective than selection by exhaustive search.

In Figure ~\ref{fig.node_removal_comparison}, we ran a comparison
that, one at a time, removed nodes from the network, using each of
these three methods for choosing which nodes to remove, calculated the
resulting epidemic size, and plotted the results.
...
Figure ~\ref{fig.node_removal_comparison} shows that selecting nodes
to vaccinate based on their degree is just as good as selecting them
by exhaustive search, and far better than a random selection
method. Furthermore, figure ~\ref{fig.node_removal_comparison} shows
that, with relatively few highly-connected nodes removed, the epidemic
size drops dramatically. The implications of this are significant: in
a college of 2,000 students, if an epidemic broke out, you could save
1000 students by vaccinating the top 100 most connected students.



\section{Tying Back to the Real World}

We now know that even if we don't have full knowledge of a network, we
can accurately determine which nodes to vaccinate by selecting the
ones that have the highest degree.

But what does having a `high degree' mean in real life?

Though campus social media gurus may have a thousand or more digital
`friends' and `followers', they would not be the key students to
vaccinate in case of an epidemic. Rather, those students who are in
close physical proximity to large numbers of people on a day-to-day
basis should be targeted.

This realization worsens the problem of pinpointing the correct
students for immunization for college officials: almost all university
students come into contact with a large number of other students on a
day-to-day basis, whether this be in the dorms, communal kitchens,
large cafeterias, or at after-hours frat parties.

\section{Further Exploration and Exercises}

In their research exploring the optimization of vaccination
distribution, Dr. Jan Medlock and Dr. Alison Galvani present a very
interesting theory about disease spread, which they present in their
paper \textit{Optimizing Influenza Vaccine Distribution}. Galvani and
Medlock propose that schoolchildren, who are most responsible for
transmission, should, not surprisingly, be prioritized for
vaccination. However, they further suggest that parents of
schoolchildren, aged 30 to 39, should also be prioritized, as they
serve as the `bridges' between their children and the rest of the
population, which ultimately allow diseases to jump between schools
and the workplace.

By looking for the bridges and most-connected nodes in a network, we
can identify the most important types of nodes to vaccinate to
minimize epidemic size, even if we don't have full knowledge of the
network topology.

\begin{exercise}
Read about the idea of bridges and see if you can find a data set that
could help you identify what some real-world bridges might be. Some
possible people you might think of are flight attendants or store
clerks, who come into physical contact with a large number of people
that may not normally interact with each other. Notice that you never
talk about a specific individual as a bridge but rather a group of
people, whether it be defined by their profession or the place they
spend time in, etc.
\end{exercise}

\begin{exercise}
When we made the decision to pursue node degree as a generalizable strategy for identifying key nodes, there were many other characteristics we could have explored. These include:

\begin{description}

    \item[Node connectivity]: Node connectivity is defined to be,
      ``The minimum number of nodes which need to be removed to
      disconnect the remaining nodes from each other.'' (Wikipedia).

    \item[Network diameter]: A network's diameter is the maximum of
      all of the shortest paths between any two nodes in the graph.

    \item[Average shortest path length in network]: The average
      shortest path length is the average of all of the shortest path
      lengths between any two nodes in the network.

    \item[Network clustering coefficient]: A network's clustering
      coefficient is, ``a measure of degree to which nodes in a graph
      tend to cluster together.'' (Wikipedia).

    \item[Link Load]: Link load is, ``The amount of traffic carried by
      a link, or by the most heavily loaded link in the system''
      (http://parallel.ru/info/reference/hpccgloss.html).

\end{description}

\end{exercise}

Many of these node and graph characteristics provide varying degrees
of effectiveness in identifying key nodes for removal in disease
spread networks. Experiment with using a few of these other
characteristics to identify important nodes for quarantine. As a
bonus, experiment with different graph topologies such as
Watts-Strogatz, Erdos-Renyi, etc. and determine which characteristics
are most effective for identifying key nodes.
